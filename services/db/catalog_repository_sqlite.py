# services/db/catalog_repository_sqlite.py
"""
SQLAlchemy ORM model and SQLite repository for test_cases table (local / non-strict mode).
"""
...
logger = logging.getLogger("vanya.db.catalog.sqlite")
...
class TestCaseRow(Base):
    ...
    source_run_id = Column(String, index=True)
...
# ── Conversion helpers (shared shape with Supabase rows) ──────────────────────
...
def test_case_to_storage_dict(tc) -> Dict[str, Any]:
    """Flat dict matching test_cases columns (SQLite insert + Supabase row)."""
...
class CatalogRepositorySqlite:
    ...
    def create_test_case(self, tc):
        data = test_case_to_storage_dict(tc)
        with get_session() as s:
            row = TestCaseRow(**data)
            s.add(row)
        logger.debug("catalog_repo.sqlite: inserted %s", tc.test_case_id)
        return tc
    ...
    def delete_test_case(self, test_case_id: str) -> bool:
        with get_session() as s:
            ...
            if row is None:
                return False
            s.delete(row)
        logger.debug("catalog_repo.sqlite: deleted %s", test_case_id)
        return True
