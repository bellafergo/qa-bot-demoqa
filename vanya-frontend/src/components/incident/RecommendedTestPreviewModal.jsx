...
import { TEST_RECOMMENDATION_I18N_KEYS } from "../../utils/testRecommendationViewUtils.js";

export default function RecommendedTestPreviewModal({ open, payload, onClose }) {
  const { t } = useLang();
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="recommended-test-preview-title"
    >
      <div className="card" style={{ width: "min(560px, 100%)", padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)" }}>
          <div id="recommended-test-preview-title" style={{ fontSize: 16, fontWeight: 600, color: "var(--text-1)" }}>
            {payload.title}
          </div>
          <div style={{ fontSize: 12, color: "var(--text-3)", marginTop: 6 }}>
            {t(TEST_RECOMMENDATION_I18N_KEYS.previewSubtitle)}
          </div>
        </div>
