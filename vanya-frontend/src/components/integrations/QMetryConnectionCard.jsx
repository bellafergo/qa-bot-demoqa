...
export default function QMetryConnectionCard({ vm }) {
  if (!vm) return null;
        ...
        <>
          <div style={{ display: "grid", gap: 8, marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
              <span style={{ color: "var(--text-3)" }}>{vm.serverUrlLabel}</span>
              <span style={{ fontFamily: "monospace", fontSize: 12 }}>{vm.serverUrl}</span>
            </div>
            {vm.lastSync ? (
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
