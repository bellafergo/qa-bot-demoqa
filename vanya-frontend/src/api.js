...
export function getMyPermissions() {
  return apiGet("/security/me/permissions");
}
...
export const getJiraIssueIntelligence   = (params = {}) => {
  const qs = new URLSearchParams();
  if (params.project_key) qs.set("project_key", params.project_key);
  const q = qs.toString();
...
export const getQMetryRecommendations   = (params = {}) => {
  ...
  if (params.project_id) qs.set("project_id", params.project_id);
  const q = qs.toString();
  return apiGet(`/integrations/qmetry/recommendations${q ? `?${q}` : ""}`);
};
...
/** GET /projects/{id}/knowledge/workflows */
export function getProjectKnowledgeWorkflows(projectId) {
  return apiGet(`/projects/${encodeURIComponent(projectId)}/knowledge/workflows`);
}
