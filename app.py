...
from api.routes.servicenow_integration_routes import router as servicenow_integration_router
...
from api.routes.enterprise_system_connector_routes import admin_router as enterprise_system_admin_router
from api.routes.enterprise_system_connector_routes import validation_router as enterprise_system_validation_router
...
from api.routes.azure_devops_project_routes import router as azure_devops_project_router
from api.routes.azure_devops_oauth_routes import router as azure_devops_oauth_router
...
def get_run_evidence(run_id: str, request: Request, format: str = "html"):
    ...
    if not run and supabase_qa_runs_enabled():
        ...
        run = fetch_qa_run_by_lookup_id(rid)

    # 3) SQLite + merged history lookup (correlation_id, evidence_id, shortcode)
    if not run:
...
app.include_router(servicenow_integration_router)  # GET /integrations/servicenow/* (read-only discovery)
...
app.include_router(enterprise_system_admin_router)     # /local-agents/enterprise-systems
app.include_router(enterprise_system_validation_router)  # /enterprise-system-validations
...
app.include_router(azure_devops_project_router) # /projects/{id}/azure-devops/*
app.include_router(azure_devops_oauth_router)   # GET /azure-devops/callback
